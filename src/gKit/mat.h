...
struct Transform
{
    ...
    static Transform Transpose(const Transform& m);
    static Transform Inverse(const Transform& m);
    static Transform Normal(const Transform& m);
    static Transform Scale(const float x, const float y, const float z);
    static Transform Translation(const Vector& v);
    static Transform TranslationXYZ(const float x, const float y, const float z);
    static Transform RotationX(const float angle);
    static Transform RotationY(const float angle);
    static Transform RotationZ(const float angle);
    static Transform Rotation(const Vector& axis, const float angle);
    static Transform Viewport(const float width, const float height);
    static Transform Perspective(const float fov, const float aspect, const float znear, const float zfar);
    static Transform Lookat(const Point& from, const Point& to, const Vector& up);
    static Transform compose_transform(const Transform& a, const Transform& b);

    LUA_BEGIN_BIND_METHODS(Transform)
        LUA_BIND_CONSTRUCTOR_NOARGS
        /*LUA_BIND_CONSTRUCTOR(_def<float, 1>, _def<float, 0>, _def<float, 0>, _def<float, 0>,
                            _def<float, 0>, _def<float, 1>, _def<float, 0>, _def<float, 0>,
                            _def<float, 0>, _def<float, 0>, _def<float, 1>, _def<float, 0>,
                            _def<float, 0>, _def<float, 0>, _def<float, 0>, _def<float, 1>)
       */
        LUA_BIND_STATIC_METHOD(Transform, Identity)
        LUA_BIND_STATIC_METHOD(Transform, Transpose)
        LUA_BIND_STATIC_METHOD(Transform, Inverse)
        LUA_BIND_STATIC_METHOD(Transform, Normal)
        LUA_BIND_STATIC_METHOD(Transform, Scale)
        LUA_BIND_STATIC_METHOD(Transform, Translation)
        LUA_BIND_STATIC_METHOD(Transform, RotationX)
        LUA_BIND_STATIC_METHOD(Transform, RotationY)
        LUA_BIND_STATIC_METHOD(Transform, RotationZ)
        LUA_BIND_STATIC_METHOD(Transform, Viewport)
        LUA_BIND_STATIC_METHOD(Transform, Perspective)
        LUA_BIND_STATIC_METHOD(Transform, Lookat)
        LUA_BIND_STATIC_METHOD(Transform, compose_transform)
