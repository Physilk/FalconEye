...
Transform Transform::Identity() { return ::Identity(); }
Transform Transform::Transpose(const Transform& m) { return ::Transpose(m); }
Transform Transform::Inverse(const Transform& m) { return ::Inverse(m); }
Transform Transform::Normal(const Transform& m) { return ::Normal(m); }
Transform Transform::Scale(const float x, const float y, const float z) { return ::Scale(x, y, z); }
Transform Transform::Translation(const Vector& v) { return ::Translation(v); }
Transform Transform::TranslationXYZ(const float x, const float y, const float z) { return ::Translation(x, y, z); }
Transform Transform::RotationX(const float angle) { return ::RotationX(angle); }
Transform Transform::RotationY(const float angle) { return ::RotationY(angle); }
Transform Transform::RotationZ(const float angle) { return ::RotationZ(angle); }
Transform Transform::Rotation(const Vector& axis, const float angle) { return ::Rotation(axis, angle); }
Transform Transform::Viewport(const float width, const float height) { return ::Viewport(width, height); }
Transform Transform::Perspective(const float fov, const float aspect, const float znear, const float zfar) { return ::Perspective(fov, aspect, znear, zfar); }
Transform Transform::Lookat(const Point& from, const Point& to, const Vector& up) { return ::Lookat(from, to, up); }
Transform Transform::compose_transform(const Transform& a, const Transform& b) { return ::compose_transform(a, b); }